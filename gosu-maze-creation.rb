  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      if @columns[cell_x][cell_y].east != nil
        if @columns[cell_x][cell_y].east.visited != true && @columns[cell_x][cell_y].east.vacant == true
          east_path = search(cell_x + 1,cell_y)
        end
      end

      if @columns[cell_x][cell_y].west != nil
        if @columns[cell_x][cell_y].west.visited != true && @columns[cell_x][cell_y].west.vacant == true
          west_path = search(cell_x - 1,cell_y)
        end
      end

      if @columns[cell_x][cell_y].south != nil
        if @columns[cell_x][cell_y].south.visited != true && @columns[cell_x][cell_y].south.vacant == true
          south_path = search(cell_x,cell_y + 1)
        end
      end

      if @columns[cell_x][cell_y].north != nil
        if @columns[cell_x][cell_y].north.visited != true && @columns[cell_x][cell_y].north.vacant == true
